...
#    self.colourpalette.setColourPalette("Teals")
  ...
  def drawBasicDataPlot(self,dataHist,luminosity,CME,xname,yname,legendlines,name,binlow=-1,binhigh=-1,doLogY=False,doLogX=False,doRectangular=False) :
    ...
    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
...
#    temp = self.drawFitHist(fitHist,firstBin,lastBin,"","",True,False,drawError, errors,drawAsSmoothCurve,-1,1,doEndLines)
    ...
    if dodrawUsersText :
      self.drawUsersText(0.605,0.72,"|y_{23}*| < 0.6",0.05)
...
#    self.drawSignificanceHist(predHist,firstBin,lastBin,xname,yname,False,False,False,False,self.colourpalette.statisticalTestFillColour)
  ...
  def drawDataWithFitAsHistogramAndResidual(self,dataHist,fitHist,luminosity,CME,xname,yname,legendlines,name,drawError=False,errors = [],residualList = [],binlow=-1,binhigh=-1,doLogY=False,doLogX=False,drawAsSmoothCurve=False,doRectangular=False,doLegTopRight=True,doLabels=True,doEndLines=False,writeOnpval = False, pval = -999, writeOnFit = False, FitMin =-999,FitMax =-999, yMin = -1, yMax = -1) :
    ...
    if yMin != -1 and yMax != -1:
      dataHist.GetYaxis().SetRangeUser(yMin,yMax)#8,5E4)
      fitHist.GetYaxis().SetRangeUser(yMin,yMax)#8,5E4)
      for errhistpair in errors :
        for thishist in errhistpair :
          thishist.GetYaxis().SetRangeUser(yMin,yMax)#8,5E4)

#    self.drawDataHist(dataHist,firstBin,lastBin,xname,yname,False,1)
#    temp = self.drawFitHist(fitHist,firstBin,lastBin,"","",True,False,drawError, errors,drawAsSmoothCurve,-1,1,doEndLines)

    #self.drawFitHist(fitHist,firstBin,lastBin,xname,yname,False,False,drawError,errors,drawAsSmoothCurve,-1,1,doEndLines,True)
    #self.drawDataHist(dataHist,firstBin,lastBin,"","",True,1,False,False,True)
    self.drawFitHist(fitHist,firstBin,lastBin,xname,yname,False,False,drawError,errors,drawAsSmoothCurve,-1,1,doEndLines,False,True)
    self.drawDataHist(dataHist,firstBin,lastBin,"","",True,1,False,True,False)
    pad1.Update()
    ...
    if dodrawUsersText:
      if writeOnFit:
        if writeOnpval:
          # AT CIRC
          #self.drawUsersText(0.17,0.42,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.039)
          self.drawMultiUsersTextMultiPositions(["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6","#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV"],[0.14,0.14,0.16,0.16],[0.93,0.88,0.39,0.335],0.035)   
        else:
          self.drawUsersText(0.17,0.42,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.039)
      else: 
        if writeOnpval:
          self.drawUsersText(0.17,0.42,"#it{p}-value = "+str(round(pval,2))+"",0.039)
        elif UserScaleText != []:
          self.drawUsersText(0.17,0.42,"|y_{23}*| < 0.6",0.039)
    #if dodrawUsersText: 
    #  self.drawUsersText(0.17,0.35,"|y_{23}*| < 0.6",0.041)
    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
    ...
    if legendsize > 0 :
      if doLogX and (not doLegTopRight) :
        ...
        legendbottom = legendtop - legendsize
        legend = self.makeLegend(leftOfLegend,legendbottom,rightOfLegend,legendtop)
      legend.AddEntry(dataHist,legendlines[0],"P")
      if drawError :
        legend.AddEntry(fitHist,legendlines[1],"L")
        if errors != [] :
          for errhist in errors :
    ...
    if (doLabels) :
      if legendsize > 0 and doLogX and not doLegTopRight:
        ...
        self.drawCMEAndLumi(0.075,0.44,CME,lumiInFb,0.034)
        #self.drawATLASLabels(0.47,0.91,False)#,True)
        if self.labeltype ==4:
          self.drawATLASLabels(0.41,0.92,False)#,True)
        else:
          self.drawATLASLabels(0.49,0.92,False)#,True)
      else :
        self.drawATLASLabels(0.2, 0.2)
...
#    temp = self.drawFitHist(fitHist,firstBin,lastBin,"","",True,False,drawError, errors,drawAsSmoothCurve,-1,1,doEndLines)
    ...
    temp = self.drawFitHist(fitHist,firstBin,lastBin,xname,yname,False,False,drawError,errors,drawAsSmoothCurve,-1,1,doEndLines, True)
    self.drawDataHist(dataHist,firstBin,lastBin,"","",True,1)
    # Lydia adding analysis cuts values to plot
    if dodrawUsersText: 
      self.drawUsersText(0.65,0.68,"|y_{23}*| < 0.6",0.045)
    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
  ...
  def drawManyOverlaidHistograms(self,histograms,names,xname,yname,name,xmin,xmax,ymin,ymax,extraLegendLines = [],doLogX=False,doLogY=True,doErrors=False,doRectangular=False,doLegend=True,doLegendLow=True,doLegendLocation="Left",doLegendOutsidePlot=False,doATLASLabel="Low",pairNeighbouringLines=False,dotLines = [],addHorizontalLines=[]) :
    ...
    #print "MAXXXXXXXXXXXXXXXXXXXXX",histograms.GetBinLowEdge(maxX)
    # Create legend
    ...
    for histogram in histograms :
      ...
      histogram.SetTitle("")
      histogram.GetXaxis().SetRange(minX,maxX)
      ...
      # Lydia FIXME
      #if doLogX :
      #  self.fixTheBloodyTickMarks(self, pad1, histogram,minX,maxX+5,minY,maxY)
...
#            legendtop = 0.90 - (0.06)*len(extraLegendLines)
  ...
  def drawPseudoExperimentsWithObservedStat(self,pseudoStatHist,observedStat,pval,pvalerr,luminosity,CME,xname,yname,name,doRectangular=False) :
    ...
    pseudoStatHist.GetYaxis().SetTitleOffset(1.2)
    ...
    legend = self.makeLegend(0.5,0.68,0.75,0.78)
    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
    ...
    # AT CIRC
    #if self.labeltype == 1:
    #  self.drawATLASLabels(0.23,0.88)
    #else:
    #  self.drawATLASLabels(0.21,0.88)
    #self.drawCMEAndLumi(0.14,0.82,CME,lumiInFb,0.04)
    #
    ## Lydia adding observedStat value to plot
    #if dodrawUsersText:
    ...
    #  self.drawATLASLabels(0.23,0.88)
    #else:
    self.drawATLASLabels(0.515,0.88)
    self.drawCMEAndLumi(0.425,0.82,CME,lumiInFb,0.04)
    
    # Lydia adding observedStat value to plot
    if dodrawUsersText:
      self.drawMultiUsersTextMultiPositions(["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],[0.15,0.15],[0.87,0.82],0.035)   
      self.drawMultiUsersTextMultiPositions2(["#it{p}-value = "+str(round(pval,2))],[0.52],[0.63],0.04)   
  ...
  def drawBumpHunterTomographyPlot(self,tomographyGraph,name,luminosity,CME,lowX,highX) :
    canvasname = name+'_cv'
    outputname = name+epsorpdf
    if saveCfile:
      Coutputname = name+'.C'
    ...
    tomographyGraph.GetXaxis().SetTitle("m_{jj} [GeV]")
    tomographyGraph.GetXaxis().SetNdivisions(999,ROOT.kTRUE)
    tomographyGraph.GetXaxis().SetNoExponent(ROOT.kTRUE)
    tomographyGraph.GetXaxis().SetMoreLogLabels(ROOT.kTRUE)
    tomographyGraph.GetXaxis().SetRangeUser(lowX,highX)
    #tomographyGraph.GetYaxis().SetTitle("Poisson PVal of Interval")
    tomographyGraph.GetYaxis().SetNdivisions(110,ROOT.kTRUE)
    tomographyGraph.GetYaxis().SetTitle("p-value")
    # AT CIRC
    #tomographyGraph.GetYaxis().SetTitleOffset(2.1);
    tomographyGraph.GetYaxis().SetTitleOffset(1.1);
    ...
    # AT CIRC
    #if self.labeltype == 1:
    #  self.drawATLASLabels(0.24, 0.24, True)
    #  self.drawCMEAndLumi(0.157,0.19,CME,lumiInFb,0.04)
    #elif self.labeltype ==4:
    #  self.drawATLASLabels(0.38, 0.27, True)
    #  self.drawCMEAndLumi(0.14,0.22,CME,lumiInFb,0.04)
    #else:
    #  self.drawATLASLabels(0.16, 0.27, True)
    #  self.drawCMEAndLumi(0.14,0.22,CME,lumiInFb,0.04)

    if self.labeltype == 1:
      self.drawATLASLabels(0.19, 0.37, True)
      self.drawCMEAndLumi(0.1,0.315,CME,lumiInFb,0.04)
    else:
      self.drawATLASLabels(0.12, 0.375, True)
      self.drawCMEAndLumi(0.1,0.315,CME,lumiInFb,0.04)
    self.drawMultiUsersTextMultiPositions(["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],[0.19,0.19],[0.26,0.2],0.038)   
  ...
  def drawDataAndFitOverSignificanceHist(self,dataHist,fitHist,significance,x,datay,sigy,name,luminosity,CME,FitMin,FitMax,firstBin=-1,lastBin=-1,doBumpLimits=False,bumpLow=0,bumpHigh=0,extraLegendLines=[],doLogX=True,doRectangular=False,setYRange=[],writeOnpval = False, pval = -999, yMin = -1, yMax = -1) :
    ...
    if yMin != -1 and yMax != -1:
      ...
      fitHist.GetYaxis().SetRangeUser(yMin,yMax)

    self.drawDataHist(dataHist,lowbin,highbin,x,datay,False,2,False,True)
    self.drawFitHist(fitHist,lowbin,highbin,"","",True,True,False,[],False,ROOT.kRed,1,False,False,True)
    ...
    if (doLogX and not doBumpLimits) :
      ...
      self.drawCMEAndLumi(0.51,0.90,CME,lumiInFb,0.04)
      bottomOfLegend = 0.78
      legend = self.makeLegend(leftOfLegend,bottomOfLegend,0.9,0.87)
    else :
      self.drawATLASLabels(0.5, 0.87, True)
      self.drawCMEAndLumi(0.41,0.82,CME,lumiInFb,0.04)
      bottomOfLegend = 0.70
...
#      toplocation = bottomOfLegend - (0.01+widthOfRow)*(index) #topOfAll - (0.03+2*widthOfRow) - (0.01+widthOfRow)*(index)
    ...
    if dodrawUsersText:
      if doBumpLimits: 
        if not writeOnpval:
          self.drawUsersText(0.605,0.63,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}",0.033)
        else:
          self.drawUsersText(0.21,0.42,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.04)
      else: 
        if not writeOnpval:
          self.drawUsersText(0.605,0.63,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}",0.033)
        else:
          self.drawUsersText(0.56,0.7,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.04)
    # Save.
    pad1.RedrawAxis()
    pad2.RedrawAxis()
    c.RedrawAxis()
  ...
  def drawDataAndFitOverSignificanceHistOld(self,dataHist,fitHist,significance,x,datay,sigy,name,luminosity,CME,FitMin,FitMax,firstBin=-1,lastBin=-1,doBumpLimits=False,bumpLow=0,bumpHigh=0,extraLegendLines=[],doLogX=True,doRectangular=False,setYRange=[],writeOnpval = False, pval = -999) :
    ...
    ## Add a few more bins on high end if we extra space		    
    #highbin = highbin+18
    ...
    if (doLogX and not doBumpLimits) :
      ...
      self.drawATLASLabels(0.51, 0.9, True)
      self.drawCMEAndLumi(0.42,0.85,CME,lumiInFb,0.04)
      bottomOfLegend = 0.74
      legend = self.makeLegend(leftOfLegend,bottomOfLegend,0.91,0.835)
...
#      toplocation = bottomOfLegend - (0.01+widthOfRow)*(index) #topOfAll - (0.03+2*widthOfRow) - (0.01+widthOfRow)*(index)
  ...
  def drawHistsOverSignificanceHists(self,histograms,names,significancehists,xname,yname,sigy,name,luminosity,CME,xmin,xmax,ymin,ymax,doLogX=True,doLogY=True,doRectangular=False,doErrMain=False,doErrSig=False,sigHistRange=[]) :
    ...
    if (doLogX) :
      ...
      self.drawATLASLabels(0.53, 0.88, True)
      self.drawCMEAndLumi(0.51,0.83,CME,lumiInFb,0.04)
        ...
        if writeOnpval:
          if doBumpLimits:
            if "it{q}" in UserScaleText and "BM" in UserScaleText:
              self.drawUsersText(0.3,0.2,"#splitline{#splitline"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{23}*| < 0.6}}}",0.045)
            else:
              self.drawUsersText(0.3,0.2,"#splitline{"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{23}*| < 0.6}}}",0.045)
          else:
            if "it{q}" in UserScaleText and "BM" in UserScaleText:
              self.drawUsersText(0.15,0.2,"#splitline{#splitline"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{23}*| < 0.6}}}",0.045)
            else:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{23}*| < 0.6}}}",0.045)
        else:
          if doBumpLimits:
            self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{23}*| < 0.6}}}",0.045)
          else:
            self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{23}*| < 0.6}}}",0.045)
      else: 
        if writeOnpval:
          ...
          #if "{" in UserScaleText:
          #  self.drawUsersText(0.23,0.61,"#splitline{#splitline"+UserScaleText+"}{|y_{23}*| < 0.6}",0.055)
            #self.drawUsersText(0.32,0.12,"#splitline{#splitline"+UserScaleText+"}{|y_{23}*| < 0.6}",0.055)
          #else:
          self.drawUsersText(0.23,0.64,"#splitline{"+UserScaleText+"}{|y_{23}*| < 0.6}",0.055)
          #self.drawUsersText(0.32,0.12,"#splitline{"+UserScaleText+"}{|y_{23}*| < 0.6}",0.055)
...
#      self.drawUsersText(0.15,0.25, UserScaleText,0.04)
    ...
    if (doLogX and not rightLegend) :
      ...
      self.drawCMEAndLumi(leftOfLegend-0.07,topOfLegend+0.015,CME,lumiInFb,0.04)
      #self.drawCMEAndLumi(0.51,0.82,CME,lumiInFb,0.04)
  ...
  def drawDataAndFitWithSignalsOverSignificances(self,dataHist,fitHist,signalsignificance,residual,signalsForSpec,signalsForSig,signalmasses,legendlist,x,datay,sigy,residy,name,luminosity,CME,firstBin=-1,lastBin=-1,doBumpLimits=False,bumpLow=0,bumpHigh=0,doLogX=True,doRectangular=False,rightLegend=False, UserScaleText = [],writeOnpval = False, pval = -999, writeOnFit = False, FitMin =-999,FitMax =-999,mcHist=None, yMin = -1, yMax = -1):
    ...
    # To make sure get max log labels
    dataHist.GetXaxis().SetNdivisions(999,ROOT.kTRUE)
    fitHist.GetXaxis().SetNdivisions(999,ROOT.kTRUE)
    residual.GetXaxis().SetNdivisions(999,ROOT.kTRUE)
    dataHist.GetXaxis().SetMoreLogLabels(ROOT.kTRUE)
    fitHist.GetXaxis().SetMoreLogLabels(ROOT.kTRUE)
    residual.GetXaxis().SetMoreLogLabels(ROOT.kTRUE)
    residual.GetXaxis().SetNoExponent(ROOT.kTRUE)
...
#    pad2.SetLeftMargin(0.2)
    ...
    if drawMC :
      ...
      #self.drawSignalOverlaidOnDataAndFit(dataHist,fitHist,signalsForSpec,signalmasses,[],luminosity,CME,datay,"",firstBin,lastBin,doLogX,True,False,False,3)
      # With leave y axis alone option so above range works
      self.drawSignalOverlaidOnDataAndFit(dataHist,fitHist,signalsForSpec,signalmasses,[],luminosity,CME,datay,"",firstBin,lastBin,doLogX,True,False,False,3,None,True)
    ...
    #      if doBumpLimits:
    #        if "it{q}" in UserScaleText and "BM" in UserScaleText:
    #          self.drawUsersText(0.3,0.2,"#splitline{#splitline"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
    #        else:
    #          self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
    #      else:
    #        if "it{q}" in UserScaleText and "BM" in UserScaleText:
    #          self.drawUsersText(0.15,0.2,"#splitline{#splitline"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
    #        else:
    #          self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
    #    else:
    #      if doBumpLimits:
    #        self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
    #      else:
    #        self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
    #  else: 
    #    if writeOnpval:
...
#      self.drawUsersText(0.15,0.25, UserScaleText,0.04)
    ...
    if doBumpLimits :
      ...
      lowYVal = signalsignificance.GetMinimum()#-0.2
      highYVal = residual.GetMaximum()+0.5
    ...
    if (doLogX and not rightLegend) :
      ...
      if self.labeltype == 0:
        ...
        self.drawATLASLabels(leftOfLegend+0.025, topOfLegend+0.065, True)
      else:
        self.drawATLASLabels(leftOfLegend-0.05, topOfLegend+0.065, True)
      self.drawCMEAndLumi(leftOfLegend-0.06,topOfLegend+0.015,CME,lumiInFb,0.04)
      #self.drawCMEAndLumi(0.51,0.82,CME,lumiInFb,0.04)
    ...
    if doBumpLimits :
      legend.AddEntry(line4,"BumpHunter interval","L")
    # No signal in legend anymore, now in LHS corner for plot in self.saveplots :
    #  index = self.saveplots.index(plot)
    #  legend.AddEntry(plot,legendlist[index],"LP")
      ...
      if dodrawUsersText:
        if writeOnFit:
          if writeOnpval:
            if doBumpLimits:
              # AT CIRC
              #self.drawMultiUsersText(0.15,0.3,[UserScaleText,"#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV","X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],0.065,0.045)  
              self.drawMultiUsersTextMultiPositions(["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6",UserScaleText[index],"#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV"],[0.15,0.15,0.2,0.15,0.15],[0.9,0.83,0.18,0.115,0.05],0.045)   
            else:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
          else:
            if doBumpLimits:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
            else:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}}",0.045)
        else: 
          if writeOnpval:
            self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#it{p}-value = "+str(round(pval,2))+"}",0.06)
          elif UserScaleText != []:
            self.drawUsersText(0.23,0.64,"#splitline{"+UserScaleText[index]+"}{|y_{23}*| < 0.6}",0.055)
      print "marker!!!!!!!!!!!!!!!!"
      m = ROOT.TMarker(318,20000, 20) 
      m.SetMarkerStyle(24) # was 20 when things were nice
      m.SetMarkerSize(1)
      m.SetMarkerColor(436) 
      m.Draw()  
      mline = self.line.Clone("mline") 
      mline.SetLineColor(436)
      mline.SetLineStyle(2)
      #mline.SetLineWidth(2)
      mline.SetX1(310); mline.SetY1(20000); mline.SetX2(326); mline.SetY2(20000)
      mline.Draw()
        ...
        if writeOnFit:
          if writeOnpval:
            if doBumpLimits:
              # AT CIRC
              #self.drawMultiUsersText(0.15,0.3,[UserScaleText,"#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV","X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],0.065,0.045)  
              self.drawMultiUsersTextMultiPositions(["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6","#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV"],[0.15,0.15,0.15,0.15],[0.9,0.83,0.115,0.05],0.045)   
            else:
              self.drawUsersText(0.15,0.2,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.045)
          else:
            if doBumpLimits:
              self.drawUsersText(0.15,0.2,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.045)
            else:
              self.drawUsersText(0.15,0.2,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{23}*| < 0.6}}",0.045)
        else: 
          if writeOnpval:
            self.drawUsersText(0.15,0.2,"#it{p}-value = "+str(round(pval,2)),0.06)
          elif UserScaleText != []:
            self.drawUsersText(0.23,0.64,"|y_{23}*| < 0.6",0.055)
  ...
  def drawMultipleFitsAndResiduals(self,dataHist,fitHistList,residualList,legendlist,x,datay,residyList,name,luminosity,CME,firstBin=-1,lastBin=-1,doBumpLimits=False,bumpLow=0,bumpHigh=0,doLogX=True,doRectangular=False,notLogY=False,lowY=11,highY=-1,doRatioLabels = False, RatioLabels =[], yMin = -1, yMax = -1) :
    ...
    # AT CIRC
    dataHist.GetYaxis().SetTitleOffset(0.85)
    #
    self.drawDataHist(dataHist,lowbin,highbin,"",datay,False,3,False,True) # was True
    ...
    for hist in fitHistList :
      ...
      # Zoom in on the data
      if yMin != -1 and yMax != -1:
        hist.GetYaxis().SetRangeUser(yMin,yMax)#8,5E4)
      # Lydia changed to non-curve option for Klaus' comment
      #self.drawFitHist(hist, lowbin, highbin, "", datay, True, True, False,[],False,\
      #        goodcolours[fitHistList.index(hist)], 1)
      # Draws as curve 
      self.drawFitHist(hist, lowbin, highbin, "", datay, True, True, False,[],True,\
              goodcolours[fitHistList.index(hist)], 1,False,False,True)
    pads[0].Update()
    ...
    if doRatioLabels:
      #self.drawMultiUsersText(0.5,0.33,RatioLabels,.13,0.03)
      # AT CIRC
      self.drawMultiUsersTextMultiPositions(["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],[0.15,0.15],[0.9,0.85],0.035)   
      self.drawMultiUsersTextMultiPositions2(RatioLabels,[0.5,0.5],[0.33,0.2],0.03)   
    ...
    if (doLogX) :
      ...
      leftOfLegend = 0.15
      if self.labeltype == 1:
        self.drawCMEAndLumi(0.44,0.83,CME,lumiInFb,0.04)
      else:
        self.drawCMEAndLumi(0.51,0.82,CME,lumiInFb,0.04)
      ...
      topOfLegend = 0.87
      leftOfLegend = 0.5
      self.drawCMEAndLumi(leftOfLegend,topOfLegend+0.02,CME,lumiInFb,0.04)
...
#      self.persistentlegend = self.makeLegend(leftOfLegend,bottomOfLegend,1.0,topOfLegend) # 0.88
    ...
    ## When doing canvas
      ...
      if self.labeltype == 0:
        ...
        self.drawATLASLabels(0.57,0.88)
      elif self.labeltype == 1:
        # AT CIRC
        #self.drawATLASLabels(0.52,0.877)
        self.drawATLASLabels(0.505,0.877)
      elif self.labeltype == 4:
        # AT CIRC
        #self.drawATLASLabels(0.38,0.877)
        self.drawATLASLabels(0.37,0.877)
      else:
        # AT CIRC
        #self.drawATLASLabels(0.58,0.88)
        self.drawATLASLabels(0.57,0.88)
...
#      boxY4 = boxY3 + (maxY - minY)*0.0155
      ...
      if self.labeltype == 1:
        # AT CIRC
        #self.drawCMEAndLumi(0.43,0.825,CME,lumiInFb,0.04)
        self.drawCMEAndLumi(0.415,0.825,CME,lumiInFb,0.04)
      else:
        # AT CIRC
        #self.drawCMEAndLumi(0.5,0.825,CME,lumiInFb,0.04)
        self.drawCMEAndLumi(0.49,0.825,CME,lumiInFb,0.04)

    # Lydia adding analysis cuts values to plot
    if dodrawUsersText and doCanvas:
      if self.labeltype == 1:
        # AT CIRC
        #self.drawUsersText(0.515,0.775,"|y_{23}*| < 0.6",0.04)
       # self.drawUsersText(0.505,0.775,"|y_{23}*| < 0.6",0.04)
        self.drawMultiUsersText(0.505,0.775,["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],0.057,0.04)  
      else:
        # AT CIRC
        #self.drawUsersText(0.585,0.775,"|y_{23}*| < 0.6",0.04)
        #self.drawUsersText(0.575,0.775,"|y_{23}*| < 0.6",0.04)
        self.drawMultiUsersText(0.575,0.775,["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],0.057,0.04)  
...
#    pad4.SetRightMargin(0.2)
    ...
    self.drawCMEAndLumi(0.47,0.80,CME,lumiInFb,0.05)
    c.Update()
...
#    savelegend5.AddEntry(plot4Materials["signal"],plot4Materials["signalLabel"],"LF")#"L")
    ...
    self.drawCMEAndLumi(0.42,0.83,CME,lumiInFb,0.05)
#    self.drawATLASLabels(0.18,0.20)
#    self.drawCMEAndLumi(0.1,0.26,CME,lumiInFb,0.04)
    c.Update()
  ...
  def draw2DLimit(self,hist,name,xAxisName,xlow,xhigh,yAxisName,ylow,yhigh,zAxisName,luminosity=-1,CME=-1,doRectangular=False) :		
    ...
    hist.GetXaxis().SetNdivisions(5,ROOT.kTRUE)
    ...
    #self.drawCMEAndLumi(0.08,0.82,CME,lumiInFb,0.04)		
    #if dodrawUsersText :
    #  self.drawUsersText(0.165,0.74,"|y_{23}*| < 0.6",0.04)

    p1 = self.drawCME(0.165,0.81,CME,0.05)		
    p2 = self.drawLumi(0.17,0.74,lumiInFb,0.05)		
    if dodrawUsersText :
      self.drawUsersText(0.165,0.695,"|y_{23}*| < 0.6",0.039)
  ...
  def drawSeveralObservedAndExpected(self,observeds,expecteds1sigma,expecteds2sigma,legendnames,name,nameX,nameY,luminosity,CME,xmin,xmax,ymin,ymax,doRectangular=False) :
    ...
    # Create legend
    leftOfLegend = 0.28
    bottomOfLegend = 0.75-0.04*(len(observeds)-1) # 0.64
...
#      observed.SetLineWidth(3)
    ...
    shadeBox.SetFillColor(self.colourpalette.twoSigmaBandColour)
    shadeBox.DrawBox(boxX1,0.7+boxY1,boxX2,1.0+boxY4)
    shadeBox.SetFillColor(self.colourpalette.oneSigmaBandColour)
    shadeBox.DrawBox(boxX1,0.75+boxY2,boxX2,0.95+boxY3)

    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
    lumiloc = min(0.45,bottomOfLegend-0.12)
    self.drawLumiAndCMEVert(0.65,lumiloc,lumiInFb,CME,0.04)
  ...
  def drawSeveralObservedAndExpectedOverlay(self,observeds,expecteds1sigma,expecteds2sigma,legendnames,name,nameX,nameY,luminosity,CME,xmin,xmax,ymin,ymax,doRectangular=False,SignalLabel='') :
    ...
    leftOfLegend = 0.2
    bottomOfLegend = 0.75-0.04*(len(observeds)-1) # 0.64
    legend = self.makeLegend(leftOfLegend,bottomOfLegend,0.68,0.92) # 0.88
...
#      observed.SetLineWidth(3)
    ...
    self.drawATLASLabels(0.20,0.27)
    #self.drawLumiAndCMEVert(0.65,lumiloc,lumiInFb,CME,0.04)
    self.drawCME(0.2,0.2, CME,0.05)
    if dodrawUsersText :
      self.drawUsersText(0.415,0.2,", "+SignalLabel,0.05)
    legend.Draw()
    ...
    shadeBox.SetFillColor(self.colourpalette.twoSigmaBandColour)
    shadeBox.DrawBox(boxX1-0.4,1.1+boxY1,boxX2-0.5,1.4+boxY4)
    shadeBox.SetFillColor(self.colourpalette.oneSigmaBandColour)
    shadeBox.DrawBox(boxX1-0.4,1.15+boxY2,boxX2-0.5,1.3+boxY3)
  ...
  def drawSignalOverlaidOnBkgPlot(self,bkgPlot,signalPlots,signalMasses,legendlist,lumi,CME,yname,name,firstBin=-1,lastBin=-1,doLogY=False,FixY=False,printCanvas=True,doRectangular=False) :
    ...
    # Create legend
    if (printCanvas) :
      lumiInFb = round(float(lumi)/float(1000),nsigfigs)
      topOfLegend = 0.81
      widthOfRow = 0.05
    ...
    for observed in signalPlots :
      ...
      for bin in range(observed.GetNbinsX()+2) :
        #if ((observed.GetBinLowEdge(bin)+observed.GetBinWidth(bin) < 0.4*mass) or\
        #   (observed.GetBinLowEdge(bin) > 1.75*mass)) :
        if ((observed.GetBinLowEdge(bin)+observed.GetBinWidth(bin) < 0.9*mass) or\
           (observed.GetBinLowEdge(bin) > 1.35*mass)) :
          continue
    ...
    if (printCanvas) :
      ...
      self.drawCMEAndLumi(0.22,0.83,CME,lumiInFb,0.04)
      legend.Draw()
  ...
  def drawSignalOverlaidOnDataAndFit(self,dataHist,fitHist,signalPlots,signalMasses,legendlist,lumi,CME,yname,name,firstBin=-1,lastBin=-1,doLogX=False,doLogY=True,printCanvas=True,doRectangular=False,nPads = 1,mcHist=None,LeaveYAxisAlone = False) :
    ...
    if drawMC :
      ...
      self.drawFitHist(fitHist,lowbin,highbin,xname,yname,False,True,False,[],False,ROOT.kRed,1,False,True,LeaveYAxisAlone)#drawMC) # Used to only extend range if incl MC on plot
                                                                                                                    # Now need to extend to fit legend in even without MC	
    ...
    for observed in signalPlots :
      ...
      for bin in range(observed.GetNbinsX()+2) :
        #if ((observed.GetBinLowEdge(bin)+observed.GetBinWidth(bin) < 0.4*mass) or\
        #   (observed.GetBinLowEdge(bin) > 1.75*mass)) :
        #if ((observed.GetBinLowEdge(bin)+observed.GetBinWidth(bin) < 0.62*mass) or\
        if ((observed.GetBinLowEdge(bin)+observed.GetBinWidth(bin) < 0.9*mass) or\
           (observed.GetBinLowEdge(bin) > 1.35*mass)) :
          continue
    ...
    self.drawDataHist(dataHist,lowbin,highbin,"","",True,nPads,False,LeaveYAxisAlone,True)#drawMC) # Used to only extend range if incl MC on plot
                                                                                         # Now need to extend to fit legend in even without MC	
    if drawMC :
      self.drawFitHist(mcHist,lowbin,highbin,xname,yname,True,True,False,[],False,ROOT.kGreen+2,1,False,drawMC)		
    self.drawFitHist(fitHist,lowbin,highbin,xname,yname,True,True,False,[],False,ROOT.kRed,1,False,True,LeaveYAxisAlone)#drawMC) # Used to only extend range if incl MC on plot
                                                                                                                 # Now need to extend to fit legend in even without MC	
    if (printCanvas) :
      ...
      # Lydia Moved labels self.drawCMEAndLumi(leftOfLegend+0.02,topOfLegend+0.02,CME,lumiInFb,0.04)
      self.drawLumiAndCMEVert(0.57,0.73,lumiInFb,CME,0.04)
      legend.SetFillStyle(0)
      legend.Draw()
  ...
  def drawSeveralObservedLimits(self,observedlist,signallegendlist,name,nameX,nameY,luminosity,CME,xmin,xmax,ymin,ymax,extraLegendLines = [], doLogY=True,doLogX=False,doRectangular=False,doLegendLocation="Right",ATLASLabelLocation="BottomL",isTomBeingDumb=False) :
    ...
    leftOfLegend = leftOfAll-0.02
    if doLegendLocation=="Left" :
      rightOfLegend = 0.50
    else :
      rightOfLegend = 0.90
    topOfLegend = topOfAll - (widthOfRow+0.05)*len(extraLegendLines)-0.09# 0.75
...
#      c.Update()
#      persistent.append(self.drawLumi(leftOfAll,topOfAll-0.02-widthOfRow,lumiInFb,0.04))
      # Lydia persistent.append(self.drawCMEAndLumi(leftOfAll-0.14,topOfAll,CME,lumiInFb,0.04))
      persistent.append(self.drawCMEAndLumi(leftOfAll-0.085,topOfAll,CME,lumiInFb,0.04))

    if dodrawUsersText :
      # Lydia self.drawUsersText(leftOfAll-0.05,topOfAll-0.05,"|y_{23}*| < 0.6",0.04)
      self.drawMultiUsersText(leftOfAll,topOfAll-0.05,["X + j (P_{T, j} > 430 GeV)","|y_{23}*| < 0.6"],0.057,0.04)  
  ...
  def drawSeveralObservedLimitsOverlay(self,observedlist,signallegendlist,name,nameX,nameY,luminosity,CME,xmin,xmax,ymin,ymax,extraLegendLines = [], doLogY=True,doLogX=False,doRectangular=False,doLegendLocation="Right",ATLASLabelLocation="BottomL",isTomBeingDumb=False) :
    # LegendLocation should be "Right","Left", or "Wide"
    # ATLASLabelLocation should be "BottomL", "BottomR", "byLegend", or "None"
    canvasname = name+'_cv'
    outputname = name+epsorpdf
    ...
    if doLegendLocation == "Wide":
      legend = self.makeLegend(0.2,0.65,0.9,0.85)
    else:
      legend = self.makeLegend(leftOfLegend,bottomOfLegend,rightOfLegend,topOfLegend)
...
#      persistent.append(self.drawLumi(leftOfAll,topOfAll-0.02-widthOfRow,lumiInFb,0.04))
      # Lydia persistent.append(self.drawCMEAndLumi(leftOfAll-0.14,topOfAll,CME,lumiInFb,0.04))
      persistent.append(self.drawCMEAndLumi(leftOfAll-0.08,topOfAll,CME,lumiInFb,0.04))
  ...
  def drawPosteriorsWithCLs(self,posteriorsandclslist,legendlist,luminosity,CME,name,align=2,central=True,drawAsHist=False,addlinestolegend=False,doLogY=False,drawPriors=[],doRectangular=False,newxname="",newyname="") :
    ...
    if align == 0 :
      ...
      self.drawCMEAndLumi(leftOfLegend-0.05,0.805,CME,lumiInFb,0.04)
    elif align == 2 :
      self.drawATLASLabels(0.53, 0.87, True)
      self.drawCMEAndLumi(leftOfLegend-0.05,0.805,CME,lumiInFb,0.04)
    else :
      self.drawATLASLabels(0.2,0.2)
      self.drawCMEAndLumi(leftOfLegend+-0.05,0.87,CME,lumiInFb,0.04)
  ...
  def drawFitHist(self,fitHist,firstBin,lastBin,xname,yname,same=False,twoPads=False,useError=False,errors = [],\
        drawCurve=False, lineColor = -1, lineStyle = 1, doEndLines = False,extraRoom=False, LeaveYAxisAlone = False ) :
  ...
  def drawSignificanceHist(self,significance,firstBin,lastBin,xname,yname,fixYAxis=False,\
    ...
    significance.GetXaxis().SetNdivisions(809,ROOT.kTRUE)
    # Lydia sig inject
    #significance.GetYaxis().SetNdivisions(806,ROOT.kTRUE)
  ...
  def drawATLASLabels(self,xstart,ystart,rightalign=False,isRectangular=False,fontSize=0.05) :
    ...
    extralength = {0:0, 1: 0, 2: -0.07, 3: 0.25, 4: 0.15, 5: -0.1, 6 : 0.12}
    if rightalign :
...
#    inputstring1 = "{0} = {1} {2}".format(mystring,lumiInFb,myfb)
  ...
  # Generic function to add multiple pieces of text to plot with y-spacing between pieces of text e.g. to write a value on it
  def drawMultiUsersText(self,xstart,ystart,texts,spacing,fontsize=0.06) : 
    self.multiwhitebox = ROOT.TPaveText()
    self.multiwhitebox.SetFillColor(0)
    #self.newwhitebox.SetFillStyle(1001)
    self.multiwhitebox.SetFillStyle(3000) # make box transparent
    self.multiwhitebox.SetTextColor(ROOT.kBlack)
    self.multiwhitebox.SetTextFont(42)
    self.multiwhitebox.SetTextAlign(11)
    self.multiwhitebox.SetBorderSize(0)

    self.multiwhitebox.SetTextSize(fontsize)
    self.multiwhitebox.SetX1NDC(0)
    self.multiwhitebox.SetY1NDC(0)
    self.multiwhitebox.SetX2NDC(1)
    self.multiwhitebox.SetY2NDC(1)
    for i,text in enumerate(texts):
      self.multiwhitebox.AddText(xstart,ystart-(i*spacing),"{0}".format(text))
    self.multiwhitebox.Draw()
    return
 
  # Generic function to add multiple pieces of text to plot with specific x-y coords e.g. to write a value on it
  def drawMultiUsersTextMultiPositions(self,texts,xpos,ypos,fontsize) : 
    self.multiwhitebox = ROOT.TPaveText()
    self.multiwhitebox.SetFillColor(0)
    #self.newwhitebox.SetFillStyle(1001)
    self.multiwhitebox.SetFillStyle(3000) # make box transparent
    ...
    for text,x,y in zip(texts,xpos,ypos):
      self.multiwhitebox.AddText(x,y,"{0}".format(text))
    self.multiwhitebox.Draw()
    return

  # Generic function to add multiple pieces of text to plot with specific x-y coords e.g. to write a value on it
  def drawMultiUsersTextMultiPositions2(self,texts,xpos,ypos,fontsize) : 
    self.multiwhitebox2 = ROOT.TPaveText()
    self.multiwhitebox2.SetFillColor(0)
    #self.newwhitebox2.SetFillStyle(1001)
    self.multiwhitebox2.SetFillStyle(3000) # make box transparent
    self.multiwhitebox2.SetTextColor(ROOT.kBlack)
    self.multiwhitebox2.SetTextFont(42)
    self.multiwhitebox2.SetTextAlign(11)
    self.multiwhitebox2.SetBorderSize(0)
    self.multiwhitebox2.SetTextSize(fontsize)

    self.multiwhitebox2.SetX1NDC(0)
    self.multiwhitebox2.SetY1NDC(0)
    self.multiwhitebox2.SetX2NDC(1)
    self.multiwhitebox2.SetY2NDC(1)

    for text,x,y in zip(texts,xpos,ypos):
      self.multiwhitebox2.AddText(x,y,"{0}".format(text))
    self.multiwhitebox2.Draw()
    return
