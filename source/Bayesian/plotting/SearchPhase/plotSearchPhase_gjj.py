...
bumpFoundVector = searchInputFile.Get("bumpFound")
...
#firstBin = basicData.FindBin(203)
#print "and another firstbin is",firstBin

# Find range
firstBin =0
while (basicBkgFrom4ParamFit.GetBinContent(firstBin)<1 and firstBin < basicBkgFrom4ParamFit.GetNbinsX()) :
  firstBin+=1
lastBin = basicBkgFrom4ParamFit.GetNbinsX()+1
while (basicBkgFrom4ParamFit.GetBinContent(lastBin-1)==0 and lastBin > 0) :
  lastBin-=1
if (firstBin > lastBin) :
  firstBin=1
  lastBin = basicBkgFrom4ParamFit.GetNbinsX()
...
myPainter.drawDataAndFitOverSignificanceHist(newbasicdata,newbasicBkgFrom4ParamFit,newresidualHist,\
        ...
        luminosity,13,fitRange[0],fitRange[1],firstBin,lastBin-1,True,bumpLowEdge/1000.0,bumpHighEdge/1000.0,[],True,False,[],True,bumpHunterPVal,5E1,5E5)
myPainter.drawDataAndFitOverSignificanceHist(newbasicdata,newbasicBkgFrom4ParamFit,newresidualHist,\
        'm_{jj} [TeV]','Prescale-weighted events','Significance','{0}/figure1_nobump'.format(folderextension),\
        luminosity,13,fitRange[0],fitRange[1],firstBin,lastBin-1,False,bumpLowEdge,bumpHighEdge,[],True,False,[],True,bumpHunterPVal,5E1,5E5)
...
myPainter.drawPseudoExperimentsWithObservedStat(bumpHunterStatHist,float(bumpHunterStatFitToData),bumpHunterPVal,0,luminosity,13,\
        'BumpHunter test statistic','Pseudo-experiments',"{0}/bumpHunterStatPlot".format(folderextension))
myPainter.drawBumpHunterTomographyPlot(bumpHunterTomographyPlot,"{0}/bumpHunterTomographyPlot".format(folderextension),luminosity,13,169,1493)

print "For Note: Bin range, N(Obs),N(exp)_Nom,N(exp_Alt)"

print 
for bin in range(newbasicdata.GetNbinsX()+2) :
   sys.stdout.write('{0}-{1}'.format(int(newbasicdata.GetBinLowEdge(bin)*1000),int(newbasicdata.GetBinLowEdge(bin+1)*1000)))
   sys.stdout.write(" & ")
   sys.stdout.write('{0}'.format(int(newbasicdata.GetBinContent(bin))))
   sys.stdout.write(" & ")
   sys.stdout.write('{0}'.format(int(newbasicBkgFrom4ParamFit.GetBinContent(bin))))
   sys.stdout.write(" & ")
   sys.stdout.write('{0}'.format(int(newAlternateBkg.GetBinContent(bin))))
   sys.stdout.write("  \n")

#if bumpFound:
#    # FIXME this is bump region not window excluded !!!!!!!!!!!!!
#    bumpHunterPLowHigh = inputFile.Get('bumpHunterPLowHigh')
#    bumpLowEdge         = bumpHunterPLowHigh[1]
#    bumpHighEdge        = bumpHunterPLowHigh[2]
#    myPainter.drawDataAndFitOverSignificanceHistExclusion(newbasicdata,newbasicBkgFrom4ParamFit,newresidualHist,\
#          'm_{jj} [TeV]','Events','Significance','{0}/figure1_exclusion'.format(folderextension),\
...
if doAlternate:
  ...
  # Make a ratio histogram for Sasha's plot GeV.
  altFitRatioGeV = ROOT.TH1D("altFitRatioGeV","altFitRatioGeV",len(standardbins)-1,array('d',standardbins))
  for bin in range(0,altFitRatioGeV.GetNbinsX()+1) :
    if basicBkgFrom4ParamFit.GetBinContent(bin) == 0 :
      altFitRatioGeV.SetBinContent(bin,0)
    else :
      altFitRatioGeV.SetBinContent(bin,(valueNewFuncErrDirected.GetBinContent(bin)-basicBkgFrom4ParamFit.GetBinContent(bin))/basicBkgFrom4ParamFit.GetBinContent(bin))
  ...
  # Make a ratio histogram for Sasha's plot GeV.
  PlusNomRatioGeV = ROOT.TH1D("PlusNomRatioGeV","PlusNomRatioGeV",len(standardbins)-1,array('d',standardbins))
  for bin in range(0,PlusNomRatioGeV.GetNbinsX()+1) :
    if basicBkgFrom4ParamFit.GetBinContent(bin) == 0 :
      PlusNomRatioGeV.SetBinContent(bin,0)
    else :
      PlusNomRatioGeV.SetBinContent(bin,(newNomPlus1.GetBinContent(bin)-basicBkgFrom4ParamFit.GetBinContent(bin))/basicBkgFrom4ParamFit.GetBinContent(bin))
  MinusNomRatioGeV = ROOT.TH1D("MinusNomRatioGeV","MinusNomRatioGeV",len(standardbins)-1,array('d',standardbins))
