...
#    temp = self.drawFitHist(fitHist,firstBin,lastBin,"","",True,False,drawError, errors,drawAsSmoothCurve,-1,1,doEndLines)
    ...
    if dodrawUsersText :
      self.drawUsersText(0.605,0.72,"|y_{12}*| < 0.8",0.05)
...
#    temp = self.drawFitHist(fitHist,firstBin,lastBin,"","",True,False,drawError, errors,drawAsSmoothCurve,-1,1,doEndLines)
    ...
    if dodrawUsersText:
      if writeOnFit:
        if writeOnpval:
          # AT CIRC
          #self.drawUsersText(0.17,0.42,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.039)
          self.drawMultiUsersTextMultiPositions(["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8","#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV"],[0.14,0.14,0.16,0.16],[0.93,0.88,0.39,0.335],0.035)   
        else:
          self.drawUsersText(0.17,0.42,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.039)
      else: 
        if writeOnpval:
          self.drawUsersText(0.17,0.42,"#it{p}-value = "+str(round(pval,2))+"",0.039)
        elif UserScaleText != []:
          self.drawUsersText(0.17,0.42,"|y_{12}*| < 0.8",0.039)
    #if dodrawUsersText: 
    #  self.drawUsersText(0.17,0.35,"|y_{12}*| < 0.8",0.041)
    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
...
#    temp = self.drawFitHist(fitHist,firstBin,lastBin,"","",True,False,drawError, errors,drawAsSmoothCurve,-1,1,doEndLines)
    ...
    # Lydia adding analysis cuts values to plot
    if dodrawUsersText: 
      self.drawUsersText(0.65,0.68,"|y_{12}*| < 0.8",0.045)
    decluminosity = Decimal(str(float(luminosity)/float(1000)))
    lumiInFb = Decimal(decluminosity.quantize(Decimal('.1'), rounding = ROUND_HALF_UP))
...
#            legendtop = 0.90 - (0.06)*len(extraLegendLines)
  ...
  def drawPseudoExperimentsWithObservedStat(self,pseudoStatHist,observedStat,pval,pvalerr,luminosity,CME,xname,yname,name,doRectangular=False) :
    ...
    #self.drawATLASLabels(0.56,0.88)
    # AT CIRC
    #self.drawCMEAndLumi(0.14,0.82,CME,lumiInFb,0.04)
    #self.drawCMEAndLumi(0.48,0.82,CME,lumiInFb,0.04)
    ...
    if dodrawUsersText:
      ...
      #self.drawUsersText(0.58,0.62,"#it{p}-value = "+str(round(pval,2)),0.04)
      self.drawMultiUsersTextMultiPositions(["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],[0.15,0.15],[0.87,0.82],0.035)   
  ...
  def drawBumpHunterTomographyPlot(self,tomographyGraph,name,luminosity,CME,lowX,highX) :
   ...
   #   self.drawATLASLabels(0.2, 0.34, True)
   #   self.drawCMEAndLumi(0.117,0.28,CME,lumiInFb,0.04)
   # elif self.labeltype ==4:
      # AT CIRC
      #self.drawATLASLabels(0.38, 0.27, True)
      #self.drawCMEAndLumi(0.14,0.22,CME,lumiInFb,0.04)
   #   self.drawATLASLabels(0.34, 0.37, True)
   #   self.drawCMEAndLumi(0.1,0.31,CME,lumiInFb,0.04)
   # else:
      # AT CIRC
      #self.drawATLASLabels(0.16, 0.27, True)
      #self.drawCMEAndLumi(0.14,0.22,CME,lumiInFb,0.04)
   #   self.drawATLASLabels(0.12, 0.37, True)
   #   self.drawCMEAndLumi(0.1,0.31,CME,lumiInFb,0.04)
    if self.labeltype == 1:
      ...
      self.drawCMEAndLumi(0.1,0.315,CME,lumiInFb,0.04)
    self.drawMultiUsersTextMultiPositions(["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],[0.19,0.19],[0.26,0.2],0.038)   
...
#      toplocation = bottomOfLegend - (0.01+widthOfRow)*(index) #topOfAll - (0.03+2*widthOfRow) - (0.01+widthOfRow)*(index)
    ...
    if dodrawUsersText:
      if doBumpLimits: 
        if not writeOnpval:
          self.drawUsersText(0.605,0.63,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}",0.033)
        else:
          self.drawUsersText(0.21,0.42,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.04)
      else: 
        if not writeOnpval:
          self.drawUsersText(0.605,0.63,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}",0.033)
        else:
          self.drawUsersText(0.56,0.7,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.04)
    # Save.
    pad1.RedrawAxis()
    pad2.RedrawAxis()
    c.RedrawAxis()
        ...
        if writeOnpval:
          if doBumpLimits:
            if "it{q}" in UserScaleText and "BM" in UserScaleText:
              self.drawUsersText(0.3,0.2,"#splitline{#splitline"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{12}*| < 0.8}}}",0.045)
            else:
              self.drawUsersText(0.3,0.2,"#splitline{"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{12}*| < 0.8}}}",0.045)
          else:
            if "it{q}" in UserScaleText and "BM" in UserScaleText:
              self.drawUsersText(0.15,0.2,"#splitline{#splitline"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{12}*| < 0.8}}}",0.045)
            else:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{12}*| < 0.8}}}",0.045)
        else:
          if doBumpLimits:
            self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{12}*| < 0.8}}}",0.045)
          else:
            self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText+"}{#splitline{Fit Range: "+str(round(FitMin/1000,1))+" - "+str(round(FitMax/1000,1))+" TeV}{|y_{12}*| < 0.8}}}",0.045)
      else: 
        if writeOnpval:
          ...
          #if "{" in UserScaleText:
          #  self.drawUsersText(0.23,0.61,"#splitline{#splitline"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
            #self.drawUsersText(0.32,0.12,"#splitline{#splitline"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
          #else:
          self.drawUsersText(0.23,0.64,"#splitline{"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
          #self.drawUsersText(0.32,0.12,"#splitline{"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
...
#    pad2.SetLeftMargin(0.2)
    ...
    for plot in self.saveplots :
      ...
      if dodrawUsersText:
        if writeOnFit:
          if writeOnpval:
            if doBumpLimits:
              if "it{q}" in UserScaleText[index] and "BM" in UserScaleText[index]:
                self.drawUsersText(0.3,0.2,"#splitline{#splitline"+UserScaleText[index]+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}}",0.045)
              else:
                # AT CIRC
                #self.drawMultiUsersText(0.15,0.3,[UserScaleText,"#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV","X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],0.065,0.045)  
                self.drawMultiUsersTextMultiPositions(["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8",UserScaleText[index],"#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV"],[0.15,0.15,0.20,0.15,0.15],[0.9,0.83,0.18,0.115,0.05],0.045)   
            else:
              if "it{q}" in UserScaleText[index] and "BM" in UserScaleText[index]:
                self.drawUsersText(0.15,0.2,"#splitline{#splitline"+UserScaleText[index]+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}}",0.045)
              else:
                self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}}",0.045)
          else:
            if doBumpLimits:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}}",0.045)
            else:
              self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}}",0.045)
        else: 
          if writeOnpval:
            self.drawUsersText(0.15,0.2,"#splitline{"+UserScaleText[index]+"}{#it{p}-value = "+str(round(pval,2))+"}",0.06)
          elif UserScaleText[index] != "":
            #if "{" in UserScaleText:
            #  self.drawUsersText(0.23,0.61,"#splitline{#splitline"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
              #self.drawUsersText(0.32,0.12,"#splitline{#splitline"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
            #else:
            self.drawUsersText(0.23,0.64,"#splitline{"+UserScaleText[index]+"}{|y_{12}*| < 0.8}",0.055)
            #self.drawUsersText(0.32,0.12,"#splitline{"+UserScaleText+"}{|y_{12}*| < 0.8}",0.055)
            # Lydia adding text to say user how much signal scaled by
        #self.drawUsersText(0.15,0.25, UserScaleText,0.04)
      print "marker!!!!!!!!!!!!!!!!"
      #m = ROOT.TMarker(318,20000, 20) 
      m = ROOT.TMarker(200,300, 20) 
      ...
      mline.SetX1(186); mline.SetY1(300); mline.SetX2(213); mline.SetY2(300)
      mline.Draw()
        ...
        if writeOnFit:
          if writeOnpval:
            if doBumpLimits:
              # AT CIRC
              #self.drawMultiUsersText(0.15,0.3,[UserScaleText,"#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV","X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],0.065,0.045)  
              self.drawMultiUsersTextMultiPositions(["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8","#it{p}-value = "+str(round(pval,2)),"Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV"],[0.15,0.15,0.15,0.15],[0.9,0.83,0.115,0.05],0.045)   
            else:
              self.drawUsersText(0.15,0.2,"#splitline{#it{p}-value = "+str(round(pval,2))+"}{#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.045)
          else:
            if doBumpLimits:
              self.drawUsersText(0.15,0.2,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.045)
            else:
              self.drawUsersText(0.15,0.2,"#splitline{Fit Range: "+str(int(FitMin))+" - "+str(int(FitMax))+" GeV}{|y_{12}*| < 0.8}}",0.045)
        else: 
          if writeOnpval:
  ...
  def drawMultipleFitsAndResiduals(self,dataHist,fitHistList,residualList,legendlist,x,datay,residyList,name,luminosity,CME,firstBin=-1,lastBin=-1,doBumpLimits=False,bumpLow=0,bumpHigh=0,doLogX=True,doRectangular=False,notLogY=False,lowY=11,highY=-1,doRatioLabels = False, RatioLabels =[], yMin = -1, yMax = -1) :
    ...
    if doRatioLabels:
      ...
      # AT CIRC
      self.drawMultiUsersTextMultiPositions(["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],[0.15,0.15],[0.9,0.85],0.035)   
...
#      boxY4 = boxY3 + (maxY - minY)*0.0155
    ...
    if dodrawUsersText and doCanvas:
      if self.labeltype == 1:
        # AT CIRC
        #self.drawUsersText(0.515,0.775,"|y_{12}*| < 0.8",0.04)
        self.drawMultiUsersText(0.505,0.775,["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],0.057,0.04)  
      else:
        # AT CIRC
        #self.drawUsersText(0.585,0.775,"|y_{12}*| < 0.8",0.04)
        self.drawMultiUsersText(0.575,0.775,["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],0.057,0.04)  
...
#    savelegend5.AddEntry(plot2Materials["signal"],plot2Materials["signalLabel"],"LF")#"L")
    ...
    if dodrawUsersText :
      self.drawUsersText(0.1,0.78,"|y_{12}*| < 0.8",0.032)
    c.Update()
...
#    self.drawCMEAndLumi(0.1,0.26,CME,lumiInFb,0.04)
  ...
  def draw2DLimit(self,hist,name,xAxisName,xlow,xhigh,yAxisName,ylow,yhigh,zAxisName,luminosity=-1,CME=-1,doRectangular=False) :		
    ...
    #if dodrawUsersText :
    #  self.drawUsersText(0.165,0.74,"|y_{12}*| < 0.8",0.04)

    p1 = self.drawCME(0.165,0.81,CME,0.05)		
    p2 = self.drawLumi(0.17,0.74,lumiInFb,0.05)		
    if dodrawUsersText :
      self.drawUsersText(0.165,0.695,"|y_{12}*| < 0.8",0.039)
...
#      persistent.append(self.drawLumi(leftOfAll,topOfAll-0.02-widthOfRow,lumiInFb,0.04))
    ...
    if dodrawUsersText :
      # Lydia self.drawUsersText(leftOfAll-0.05,topOfAll-0.05,"|y_{12}*| < 0.8",0.04)
      self.drawMultiUsersText(leftOfAll,topOfAll-0.05,["X + #gamma (P_{T, #gamma} > 150 GeV)","|y_{12}*| < 0.8"],0.057,0.04)  
