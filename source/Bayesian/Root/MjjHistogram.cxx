...
// ---------------------------------------------------------
MjjHistogram::MjjHistogram(TH1D * inputHisto) :
  ...
  fRandomNumberGenerator(0)
{
  ...
  StoreEffectiveHistAndErrors();
}

MjjHistogram::MjjHistogram(TH1D * inputHisto, int seed) :
  fHistogram(*(TH1D*) inputHisto),
  fEffectiveHistogram(*(TH1D*) inputHisto),
  fWeightsHistogram(*(TH1D*) inputHisto),
...
void MjjHistogram::StoreEffectiveHistAndErrors() 
{
  ...
  for (int bin=0; bin<=fHistogram.GetNbinsX()+1; bin++) {
    ...
    if (AreIdentical(weff,double(1.0))) {
