...
#      persistent.append(self.drawLumi(leftOfAll,topOfAll-0.02-widthOfRow,lumInFb,0.04))
  ...
  # Emma's version ;)
  def drawSeveralObservedExpectedLimits(self,observedlist,expectedlist,expected1list,expected2list,signallegendlist,name,nameX,nameY,luminosity,CME,xmin,xmax,ymin,ymax,extraLegendLines = [], doLogY=True,doLogX=False,doRectangular=False,doLegendLocation="Right",ATLASLabelLocation="BottomL",addHorizontalLines=[],cutLocation="Right",labels=[]) :
    ...
    for index, observed in enumerate(observedlist) :
      ...
      def formatObservedGraph(obs,i=0):
        if isinstance(obs, (list, tuple)): [formatObservedGraph(g,j) for j,g in enumerate(obs) if g != None]
          ...
          obs.SetMarkerSize(0.7)  # was 0.5 back when things were nice
          obs.SetMarkerStyle(24+index) # was 20 when things were nice
          #if not isTomBeingDumb :
          #  obs.SetMarkerSize(0.7)  # was 0.5 back when things were nice
          #  obs.SetMarkerStyle(24+index) # was 20 when things were nice
          #else :
          #  obs.SetMarkerSize(1.0)  # was 0.5 back when things were nice
          #  obs.SetMarkerStyle(20+index) # was 20 when things were nice
      ...
      if isinstance(observed, (list, tuple)):
        for g in observed:
          if g == None: continue
          legend.AddEntry(g,signallegendlist[index],"PL")
          break

      else: legend.AddEntry(observed,signallegendlist[index],"PL")
...
#      c.Update()
#      persistent.append(self.drawLumi(leftOfAll,topOfAll-0.02-widthOfRow,lumInFb,0.04))
      if luminosity > 0: persistent.append(self.drawCMEAndLumi(leftOfAll-0.08,topOfAll,CME,lumInFb,0.04))
      else: persistent.append(self.drawCME(leftOfAll,topOfAll,CME,0.04))
      
    '''if len(labels) > 0:
      print labels
      for i,t in enumerate(labels):
        persistent.append(self.drawUsersText(0.20+i*0.4,0.2,t,0.04))
    '''
    if dodrawUsersText :
      
      if cutLocation == "BottomL" :
        self.drawUsersText(0.17,0.2,self.cutstring,0.04)
      elif not cutLocation == "Left" :
        self.drawUsersText(leftOfAll+0.01,topOfAll-0.05,self.cutstring,0.04)
...
#    inputstring1 = "{0} = {1} {2}".format(mystring,lumiInFb,myfb)
  ...
  def drawCMEAndLumi(self,xstart,ystart,CME,lumiInFb,fontsize=0.05) :
    ...
    if doLumiInPb:
      ...
      mytext = "{0}={1} TeV, {2} {3}".format(mysqrt,CME,lumiInFb,myfb)
