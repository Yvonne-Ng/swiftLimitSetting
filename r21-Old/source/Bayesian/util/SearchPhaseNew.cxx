...
int main (int argc,char **argv)
{
		...
		if (string(argv[ip]).substr(0,2)=="--") {
			...
			if (string(argv[ip])=="--config") {
				if (ip+1<argc && string(argv[ip+1]).substr(0,2)!="--") {
                    ...
                    cout<<"config json file name"<<configFile<<endl;
					ip+=2;
    ...
    //Replacing this by json
	// Get config file
    boost::property_tree::ptree pt;
    boost::property_tree::read_json((string)configFile, pt);

    cout<<"test"<<endl;
    TString inputDir = pt.get<string>("inputRootDir");
    inputFileName =pt.get<string>("inputRoot");
    inputFileName=inputDir+"/"+inputFileName;

    cout<<"inputFileName"<<inputFileName;
    outputFileName = pt.get<string>("configName");
    inputHistDir= pt.get<string>("inputTDir");
    double Ecm = pt.get<double>("ECM");
    //PermitWindow->is this still a needed variable?
    bool doPValWithSysts=pt.get<bool>("doPValWithSysts");
    int nPseudoExp=pt.get<int>("nPseudoExp");
    bool permitWindow = pt.get<bool>("permitWindow");
    dataMjjHistoName=pt.get<string>("inputHisto");
...
/*
    ...
    */
	...
	// Range for data fit
    double minX=pt.get<double>("minXForFit");
    double maxX=pt.get<double>("maxXForFit");
    int functionNumber = pt.get<int>("functionCode");
    int nPars = pt.get<int>("nParameters");
    bool doPEOnData = pt.get<bool>("doPEOnData");
...
*/
	...
	for (int par = 1; par<nPars+1; par++) {
        ...
        double param=pt.get<double>(title.c_str());
		...
		//bool isFixed = pt.get<bool>(title.c_str());
		//areParamsFixed.push_back(isFixed);
		std::cout << "   (" << par-1 << ") :" << param;
	}
	std::cout << std::endl;

    bool doAlternate = pt.get<bool>("doAlternateFunction");
    int alternateFuncNumber = pt.get<int>("alternateFunctionCode");
    int altNPars = pt.get<int>("alternateNParameters");
	...
	if (doAlternate) {
        ...
        */
		for (int par = 1; par < altNPars+1; par++) {
            ...
            //param = settings->GetValue(title.c_str()
			//double param = settings->GetValue(title.c_str(),1.0);
            double param=pt.get<double>(title.c_str());
			altParDefaults.push_back(false);
	...
	for (int par = 0; par<nPars; par++)
		...
		theMjjFitFunction->GetParameter(par)->SetFixParameter(paramDefaults.at(par));
