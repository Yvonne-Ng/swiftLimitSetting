...
doGaussianLimits = True # Set to True to run doGaussianLimits.cxx

doPlotting = False # Set to True to run plotGaussians.py

lumis = ['15p45']

ratios = [-1,0.07,0.10]#,0.15] # -1 is resolution width!

subranges = [[250,350],[350,450],[450,550],[550,650]]

plotextension = "trijet_data_hist_20160727_15p45fb_3Par_303_611"
...
config = 'configurations/Step4_GenericGaussians.config'

useBatch = False
templatescript = './scripts/OxfordBatch/Step2_BatchScript_Template_Oxford.sh'

templatefile = "%s/Bayesian/results/Step1_SearchPhase/trijet_data_hist_20160727_15p45fb_3Par_303_611/Step1_SearchPhase_Zprime_mjj_var_DataLike_LLLfb.root"%headdir
...
#-------------------------------------
# Plotting results of running doGaussianLimits.cxx using plotGaussians.py
#-------------------------------------
...
if doPlotting:
  ...
  for lumi in lumis :
    ...
    # open modified plotGaussians.py (fout) for writing
    fout = open('plotting/LimitSettingPhase/plotGaussians_%sfb.py'%(lumi), 'w')

    # read in plotGaussians fin
    with open('./plotting/LimitSettingPhase/plotGaussians.py', 'r') as fin:
      for line in fin:
    ...
    # do plotting locally
    subprocess.call("python plotting/LimitSettingPhase/plotGaussians_%sfb.py -b"%(lumi), shell=True)
    os.remove("./plotting/LimitSettingPhase/plotGaussians_%sfb.py"%(lumi)) # Remove modified plotGaussians after plotting
