...
SearchPhaseresults = "%s/Bayesian/results/Step1_SearchPhase/trijet_data_hist_20160727_15p45fb_3Par_303_611/Step1_SearchPhase_Zprime_mjj_var_DataLike_LLLfb.root"%headdir
                   # Can replace lumi in name with LLL and will be filled by Lumis value (must do this if running on multiple lumis)

##---------------------------
# Analysis quantities

lumiUnc = 0.029 # Size of luminosity uncertainty FIXME VALUE ONLY HERE FOR REFERENCE, actual value set in config, this script doesn't update config file, could make script update config like for other values!ty uncertainty FIXME make script update config!
...
plotextension = "trijet_data_hist_20160727_15p45fb_3Par_303_611"  # folder name to store all plots, results, configs and outputs consistently
...
LimitSettingPhaseconfig = "./configurations/Step3_LimitSettingPhase_MMM_%s.config"%Coupling # Path/file of LimitSettingPhase config
signalFileName = "inputs/NoTails/Chopped_303_611_hist_20160727/Chopped_MMM%d{0}.root".format(Coupling)
