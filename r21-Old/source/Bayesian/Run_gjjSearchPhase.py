...
folderextension = "testSignalPlusBkg"

#inputFileDir = "./inputs/"
inputFileDir="/lustre/SCRATCH/atlas/ywng/r21/r21Rebuild/input/"
#inputFileDir="/lustre/SCRATCH/atlas/ywng/r21/r21Rebuild/input/btagged/"
...
for File, HistList in fileHistDict.iteritems():
  for Hist in HistList:
    ...
    #Yvonne hard set of the luminosity
      ...
      fout = open('plotting/SearchPhase/plotSearchPhase%s.py'%Hist, 'w')
        ...
        for line in fin:
          if (line.startswith("searchInputFile") or line.startswith("folderextension") or line.startswith("luminosity") or line.startswith("Ecm")): 
            ...
            if line.startswith("searchInputFile"):
              ...
              #testing to see if the r21SwiftBunny graph can be drawn correctly here
              #line = "searchInputFile = ROOT.TFile('/lustre/SCRATCH/atlas/ywng/WorkSpace/r21/r21SwiftNew/r21StatisticalAnalysis/source/results/Step1_SearchPhase/svnCodebkgndFit-Redo/Step1_SearchPhase_Zprime_mjj_var.root')\n"
              line = "searchInputFile = ROOT.TFile('/lustre/SCRATCH/atlas/ywng/WorkSpace/r21/r21SwiftNew/r21StatisticalAnalysis/source/results/Step1_SearchPhase/swiftCodeTestingWithScale//Step1_SearchPhase_Zprime_mjj_var.root')\n"
#              line = "searchInputFile = ROOT.TFile('/lustre/SCRATCH/atlas/ywng/WorkSpace/r21/r21-Old/source/Bayesian/results/Step1_SearchPhase/test/Step1_SearchPhase_Zprime_mjj_var.root')\n"
              fout.write(line)
      ...
      subprocess.call("python plotting/SearchPhase/plotSearchPhase%s.py -b"%Hist, shell=True)
      #os.remove("./plotting/SearchPhase/plotSearchPhase_gjj_%s.py"%Hist)
      os.remove("./plotting/SearchPhase/plotSearchPhase%s.py"%Hist)
