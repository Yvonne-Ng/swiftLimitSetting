...
#SearchPhaseresults ="/lustre/SCRATCH/atlas/ywng/r21/r21SwiftNew/r21StatisticalAnalysis/source/results/Step1_SearchPhase/svnCodebkgndFit/Step1_SearchPhase_Zprime_mjj_var.root"
...
Lumis = ["35p45"] # Luminosities to scale limits to in fb e.g. "10" or 0p1, should have corresponding search phase ran for this lumi 
...
nPETotal = 100 #Total number of pseudo-experiments to run for expected limit bands
...
plotextension ="svnCodebkgndFit"  # folder name to store all plots, results, configs and outputs consistently
...
#outName = "JDMPh100_Zprime0p4"
##

#Coupling="gSM0p2"
#Ph="100"
#Signals["ZPrimemR"]=["250", "550", "350", "750", "450"  ]
#outName = "JDMPh100_Zprime0p2"
#
Coupling="gSM0p1"
Ph="100"
Signals["ZPrimemR"]=["250", "550", "350", "450"]
outName = "JDMPh100_Zprime0p1"
#
#Coupling="gSM0p2"
#Ph="50"
      ...
      for Lumi in Lumis:
        ...
        for split in range (0,nSplits):
          ...
          if doPDFAccErr and doISRAccErr: # do PDF acceptance error and ISR acceptance error
            command = "setLimitsOneMassPoint --config %s/Step2_%s%s_%sfb.config --mass %s --PDFAccErr %f --ISRAccErr %f --seed %i --outfile %s 2>/dev/null|& tee %s/Step2_%s%s_%sfb_%i_withPDFAndISRAccErr.txt"%(Step2_ConfigArchive,outName,Mass,Lumi,Mass,PDFErrSize,ISRErrSize,Seed,outfile,Step2_CodeOutput,outName,Mass,Lumi,split)

          elif doPDFAccErr: # do PDF acceptance error
            command = "setLimitsOneMassPoint --config %s/Step2_%s%s_%sfb.config --mass %s --PDFAccErr %f --seed %i --outfile %s 2>/dev/null|& tee %s/Step2_%s%s_%sfb_%i_withPDFAccErr.txt"%(Step2_ConfigArchive,outName,Mass,Lumi,Mass,PDFErrSize,Seed,outfile,Step2_CodeOutput,outName,Mass,Lumi,split)

          elif doISRAccErr: # do ISR acceptance error
            command = "setLimitsOneMassPoint --config %s/Step2_%s%s_%sfb.config --mass %s --ISRAccErr %f --seed %i --outfile %s 2>/dev/null|& tee %s/Step2_%s%s_%sfb_%i_withISRAccErr.txt"%(Step2_ConfigArchive,outName,Mass,Lumi,Mass,ISRErrSize,Seed,outfile,Step2_CodeOutput,outName,Mass,Lumi,split)

          else: # do not do PDF or ISR acceptance error
            command = "setLimitsOneMassPoint --config %s/Step2_%s%s_%sfb.config --mass %s --seed %i --outfile %s  2>/dev/null |& tee %s/Step2_%s%s_%sfb_%i.txt"%(Step2_ConfigArchive,outName,Mass,Lumi,Mass,Seed,outfile,Step2_CodeOutput,outName,Mass,Lumi,split)
  ...
  for Model in Signals.keys() :
    for Lumi in Lumis:
      if doLimitSettingPhase: 
        ...
        LimitSettingPhaseconfig=LimitSettingPhaseconfig.replace("PPP",Ph)
