...
# FIXME
SearchPhaseresults = "%s/Bayesian/results/Step1_SearchPhase/Data_LHCP/Step1_SearchPhase_mjj_Data_PeriodD_0p08fb.root"%headdir
...
plotextension = "Data_LHCP"
#plotextension = "Data_3p57invfb_IBLOff_Thousand"  # folder name to store all plots, results, configs and outputs consistently
#plotextension = "Data_3p34invfb_Partial"  # folder name to store all plots, results, configs and outputs consistently
...
signalFileName = "inputs/Full_25ns_20150813/QStar/StatisticalHists/1fb/MMM%d_1fb.root"
...
# RUN this using splits as want 1000 PEs not 100! 
# WPrime
...
# ZPrime   # Two new files: gSM0p10 2000 and gSM0p30 3500
# Remember to uncomment: LimitSettingPhaseconfig and signalFileName below !!!!!!

#Coupling = "gSM0p10" 
##Signals["ZPrimemR"]=["1000","1500"] 
## Temp Signals["ZPrimemR"]=["2000","2500","3000","3500"] 
## Comb
#Signals["ZPrimemR"]=["1000","1500","2000","2500","3000","3500"] 
#outName = "ZPrime0p10"

#Coupling = "gSM0p20" 
##Signals["ZPrimemR"]=["1000","1500","2000","2500"] 
##Temp Signals["ZPrimemR"]=["3000","3500"] 
##Comb 
#Signals["ZPrimemR"]=["1000","1500","2000","2500","3000","3500"] 
#outName = "ZPrime0p20"

#Coupling = "gSM0p30" 
##Signals["ZPrimemR"]=["1500","2000","2500"] 
## Temp Signals["ZPrimemR"]=["3000","3500"] 
## Comb 
...
#Coupling = "gSM0p40" 
##Signals["ZPrimemR"]=["2000","2500","3000"] 
## Temp Signals["ZPrimemR"]=["3500"] 
## Comb 
#Signals["ZPrimemR"]=["2000","2500","3000","3500"] 
#outName = "ZPrime0p40"

#Coupling = "gSM0p50" 
##Signals["ZPrimemR"]=["2500","3000"] 
## Temp Signals["ZPrimemR"]=["3500"] 
## Comb 
...
# **** TRUTH **** ZPrime NO JES AS TRUTH 
#Coupling = "gSM0p10" 
#Signals["ZPrime"]=["1500","2000","2500","3000","3500","4500"] # Signals you're using e.g. QStar, followed by mass points for the signal
#outName = "ZPrime"
...
#Coupling = "gSM0p30" 
#Signals["ZPrime"]=["1500","2000","2500","3000","3500"] # Signals you're using e.g. QStar, followed by mass points for the signal
#outName = "ZPrime"

#Coupling = "gSM0p40" 
#Signals["ZPrime"]=["2000","2500","3000","3500"] # Signals you're using e.g. QStar, followed by mass points for the signal
#outName = "ZPrime"

#Coupling = "gSM0p50" 
#Signals["ZPrime"]=["2500","3000","3500"] # Signals you're using e.g. QStar, followed by mass points for the signal
#outName = "ZPrime"
    ...
    for Mass in sorted(Signals[Model]):
      for Lumi in Lumis:
        ...
        with open('%s'%setLimitsOneMassPointconfig.replace("MMM",Model), 'r') as fin:
          for line in fin:
            if (line.startswith("dataFileName") or line.startswith("signalFileName") or line.startswith("nominalSignalHist") or line.startswith("outputFileName") or line.startswith("plotDirectory") or line.startswith("plotNameExtension") or line.startswith("signame") or line.startswith("nominalTemplateJES") or line.startswith("nameTemp")): 
        ...
        if useBatch:
          if atOx:
            ...
            fbatchout = open('%s/Step2_BatchScript_Template_%s%s_%sfb.sh'%(Step2_ScriptArchive,outName,Mass,Lumi),'w')
            ...
            fbatchout.close()
            subprocess.call("qsub < %s/Step2_BatchScript_Template_%s%s_%sfb.sh"%(Step2_ScriptArchive,outName,Mass,Lumi), shell=True)
          else:
            # Perform setLimitsOneMassPoint on batch
            print "Batch!!"
            batchcommand = command.split("|&")[0]
            ...
            modcommand = 'chmod 744 %s/Step2_BatchScript_Template_%s%s_%sfb.sh'%(Step2_ScriptArchive,outName,Mass,Lumi)
            print modcommand
            subprocess.call(modcommand, shell=True)
            subprocess.call("ls -l {0}".format(Step2_ScriptArchive), shell=True)
 
            fbatchout.close()
            command = "bsub -q 2nd %s/Step2_BatchScript_Template_%s%s_%sfb.sh"%(Step2_ScriptArchive,outName,Mass,Lumi)
            print command
            subprocess.call(command, shell=True)
    ...
    for Lumi in Lumis:
      if doLimitSettingPhase: 
        ...
        with open('%s'%LimitSettingPhaseconfig.replace("MMM",Model), 'r') as fin:
          for line in fin:
            if (line.startswith("inputFileForm") or line.startswith("outputFileName") or line.startswith("luminosity")): 
              if line.startswith("inputFileForm"): # Used %%d in line below as %d is special character in python so need extra % to allow %d
                line = "inputFileForm ./results/Step2_setLimitsOneMassPoint/%s/Step2_setLimitsOneMassPoint_%s%%d_%sfb.root\n"%(plotextension,outName,Lumi) 
                fout.write(line)
              if line.startswith("outputFileName"): 
