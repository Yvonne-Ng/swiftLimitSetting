...
#-> throw this into a function if possible
        windowWidth=12
        print ("gh 1")
        #for each window with/signal width do the below
        ROOT.gROOT.SetBatch(True)
        c1=TCanvas()
        NSMassPoint=TH1F("NoSignal", "NoSignal",len(config["lowBinEdge"])-1, array.array("f",config["lowBinEdge"]))
        ...
        for mass in config["massRange"]:
            noSignalRootFile=findLabelledFileName(args.rootFileDir, "NOSIGNAL", mass, windowWidth)
            fBkg=TFile.Open(noSignalRootFile)
            hBkgEstimate=fBkg.Get("basicBkgFrom4ParamFit")
            justBelowRootFile= findLabelledFileName(args.rootFileDir, "JUSTBELOW", mass, windowWidth)
...
#----drawing on pad 2 (the difference )
        ...
        diffGraphError.Draw("EP3 same")

        diffInMassPoint.SetMarkerStyle(5)
        diffGraph.SetMarkerColor(2)
        diffGraph.Draw("same")
        ...
        leg2.AddEntry(diffGraphError,"\"No Signal\" Bkg Pred. Error","fp")
        leg2.AddEntry(diffGraph,"\"Just Below\"- \"No Signal\" Bkg Pred","pl")
        leg2.Draw("same")

        print ("gh 7")
        c1.SaveAs("/lustre/SCRATCH/atlas/ywng/WorkSpace/r21/r21SwiftNew/SensitivityStudies/source/scripts/test.pdf")
