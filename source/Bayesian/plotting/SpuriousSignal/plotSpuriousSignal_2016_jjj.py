...
####### Get input #########
filenametemp = "/home/beresford/TriJet/StatisticalAnalysis/Bayesian/results/Step1_SearchPhase/Spurious_trijet_tla_mc_hist_20160711_6p69fb_3Par/Step1_SearchPhase_Zprime_mjj_var_Scaled_6p69fb.root"

folder = "./plotting/SpuriousSignal/plots/SpuriousSignal_trijet_tla_mc_20160711_6p69fb_3Par"

signalFolder = "/home/beresford/TriJet/StatisticalAnalysis/Bayesian/inputs/xsecandacceptance/"
signalDir = "trijet_j430_2j25"
sampleChoices = ["ZPrimemR350gSM0p30","ZPrimemR450gSM0p30","ZPrimemR550gSM0p30"] 


# Get Scaled results for MC stat uncert
initialInputFile = ROOT.TFile.Open("./inputs/hist_20160711/OUT_dijetjet_tla/datalike/hist.root","READ") # Need original scaled file as basicData in searchInputFile has root N errors
MCScaledHist = initialInputFile.Get(signalDir+"/Zprime_mjj_var_Scaled_6p69fb")
...
# Find range
firstBin = basicData.FindBin(lowfit)
lastBin = basicData.FindBin(highfit)
...
for sample in sampleChoices :
  insigfile = ROOT.TFile(signalFolder+"ZPrime_jjj.root",'r')
  print "Getting"
  #print insigfile.ls()
...
myPainter.drawManyOverlaidHistograms(histograms,names,"m_{jj} [GeV]","Relative difference from fit",figname,nomPlus1.FindBin(lowfit),nomPlus1.FindBin(highfit)-1,0.,0.02,extraLegendLines = [],doLogX=True,doLogY=False,doErrors=False,doRectangular=True,doLegend=True,doLegendLow=False,doLegendLocation="Right",doLegendOutsidePlot=False,doATLASLabel="Right",pairNeighbouringLines=False,dotLines = [],addHorizontalLines=[])
...
#myPainter.drawManyOverlaidHistograms(histograms,names,"m_{jj} [GeV]","Relative difference from fit",figname,nomPlus1.FindBin(lowfit),nomPlus1.FindBin(highfit)-1,"automatic","automatic",extraLegendLines = [],doLogX=True,doLogY=False,doErrors=False,doRectangular=True,doLegend=True,doLegendLow=False,doLegendLocation="Right",doLegendOutsidePlot=False,doATLASLabel="Right",pairNeighbouringLines=False,dotLines = [],addHorizontalLines=[])
myPainter.drawManyOverlaidHistograms(histograms,names,"m_{jj} [GeV]","Relative difference from fit",figname,nomPlus1.FindBin(lowfit),nomPlus1.FindBin(highfit)-1,0.,0.03,extraLegendLines = [],doLogX=True,doLogY=False,doErrors=False,doRectangular=True,doLegend=True,doLegendLow=False,doLegendLocation="Right",doLegendOutsidePlot=False,doATLASLabel="Right",pairNeighbouringLines=False,dotLines = [],addHorizontalLines=[])
