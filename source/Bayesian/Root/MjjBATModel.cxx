...
int MjjBATModel::AddProcess(std::string name, vector<double> min, vector<double> max)
{
   ...
   MjjBATProcess * process = new MjjBATProcess(name,true,false);
...
int MjjBATModel::AddProcess(std::string name, bool doNormUnc, bool doStatUnc, double nmin, double nmax)
{
   ...
   // this comes from a function so it should have a normalisation
   // uncertainty but no uncorrelated uncertainty.
   MjjBATProcess * process = new MjjBATProcess(name,doNormUnc,doStatUnc);
...
int MjjBATModel::PrintSummary(std::string filename)
{
   ...
   if(!ofi.is_open()) {
      BCLog::OutWarning(Form("MjjBATModel::PrintSummary() : Could not open file %s", filename));
      return 0;
   }
...
//   std::cout << "Keeping bin range [" <<lowbin << ", " << highbin << "]" << std::endl;
...
//  double n = std::max(parameters.at(GetParameter("dummyN")->GetIndex()),0.0);
  ...
  if (!(GetProcess(0)->GetDoStatUnc()) && !(GetProcess(0)->GetDoNormalisationUnc()))
