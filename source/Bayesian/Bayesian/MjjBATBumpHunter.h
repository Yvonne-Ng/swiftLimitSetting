#ifndef MJJBATBUMPHUNTER_H
#define MJJBATBUMPHUNTER_H

/*!
 * \class MjjBATBumpHunter
...
// ---------------------------------------------------------
class MjjBATBumpHunter : public MjjStatisticalTest 
{
   ...
   public:
      ...
      /**
       * Constructor including an interface to BAT.
       * The model should be set up in advance with
       * all systematic uncertainties to be considered
       * on a single process. */
      MjjBATBumpHunter(MjjBATModel * fitter);
      ...
      TGraphErrors GetWillTomography()
          { return fWillTomography; };
  
      std::vector<TH1D> GetPosteriors()
          { return posteriors; };

      std::vector<TH1D> GetWillPosteriors()
          { return willposteriors; };
       ...
       * No background is specified since that was included
       * with the model.
   ...
   private:
      ...
      double GetBATProbability(int firstBin, int lastBin, double nData, double nBNom, double nBErr);

      double GetPoissonConvGammaPValue(double nObs, double E, double err);
  
      double GetPoissonPValue(double nObs, double E);
      ...
      /**
       * An MjjBATModel for handling converting nuisance parameters
       * into expected numbers of events */
      MjjBATModel * fFitter;
  
      /** 
       * An empty histogram to hold the place of the dummy data normalization */
      TH1D * fDummyHist;
  
      /**
       * Hold the posteriors so i can tak ea look at them. */
      std::vector<TH1D> posteriors;
  
      /**
       * Hold the posteriors so i can tak ea look at them. */
      std::vector<TH1D> willposteriors;

      /**
       * Hold differences between what I calculate and will's result. 
       * Only important with everything except stat/norm deactivated. */
      std::vector<double> differences;

      vector<double> fWillProbBumps;
      TGraphErrors fWillTomography;
