...
# Overwritten by calculateGaussiansLimits_gjj.py
luminosity = 3230 #4000 #80 # ipb. 7 ipb = 0.007 ifb.

# Get input: basic gaussian
ratios = [-1,0.07,0.10]# Don't include widest gaussian,0.15]#,0.30]
...
for graph in graphs :
  ...
  for np in range(newgraph.GetN()) :
    ...
    # Efficiency not divided out
    #newgraph.SetPoint(np,d1,d2) # No divide by 1000, as want GeV
    # Efficiency divided out
    #newgraph.SetPoint(np,d1/1000.0,d2/0.81) # Dividing by 0.81 to divide out efficiency NOTE
    newgraph.SetPoint(np,d1,d2/0.81) # Dividing by 0.81 to divide out efficiency NOTE No divide by 1000, as want GeV
...
myPainter.drawSeveralObservedLimits(shiftedgraphs,names,folderextension+"GenericGaussians"+plotextension,"m_{G} [GeV]",\
     "#sigma #times #it{A} #times BR [pb]",luminosity,13,169,trueMaxRange,1E-3,1,[])#5E-2,15,[])
...
for mass in sorted(mostMasses) :
   ...
   for width in sorted(results.keys()) :
     if mass in results[width].keys() :
       ...
       # Efficiency divided out
       #sys.stdout.write('{0}'.format(round(results[width][mass]/0.81,3)))# Dividing by 0.81 to divide out efficiency NOTE 
       sys.stdout.write('{0}'.format(float('%.2g' % (results[width][mass]/0.81)))) # Dividing by 0.81 to divide out efficiency NOTE 
     else :
       sys.stdout.write("-")
...
for mass in sorted(mostMasses) :
   ...
   for width in sorted(results.keys()) :
     if mass in results[width].keys() :
       # Efficiency not divided out
       #sys.stdout.write('{0}'.format(float('%.5g' % (results[width][mass])))) 
       #sys.stdout.write("{0}".format(results[width][mass]))
       # Efficiency divided out
       sys.stdout.write('{0}'.format(float('%.5g' % (results[width][mass]/0.81)))) # Dividing by 0.81 to divide out efficiency NOTE 
     else :
       sys.stdout.write("-")
     if sorted(results.keys())[-1]!=width :
       sys.stdout.write(" & ")
